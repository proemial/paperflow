...
type Props = {
  id?: string;
  arxivOutput: ParsedArxivItem;
  modelOutput?: WithTextAndUsage;
  modelOutputString?: string;
  age?: number;
};
  ...
  return (
    <Card variant="outlined" sx={{ maxWidth: 320 }}>
      <Typography level="h2" sx={{ fontSize: 'sm' }}>
        ...
        <Box sx={{ fontSize: 'xs', color: 'grey' }}>
          {age ? `Published ${age} days ago` : ''}
        </Box>
      </Typography>
                ...
                {modelOutput && (
                  <Tooltip
                    title={
                      ...
                      >
                        <div>
                          Prompt tokens: {modelOutput.usage?.prompt_tokens}
                        </div>
                        <div>
                          Completion tokens:{' '}
                        ...
                        <div style={{ fontWeight: 'bold' }}>
                          Total tokens: {modelOutput.usage?.total_tokens}
                        </div>
                      </Box>
                    }
                    variant="outlined"
...
function WithTags({ text }: { text?: string }) {
  ...
  if (!text?.includes('#')) {
    return (
      <>
        {text.split('\n\n').map((line, i) => (
          <span key={i} style={{ display: 'block', paddingBottom: 12 }}>
            {line}
          </span>
        ))}
      </>
          ...
          return (
            <span key={i}>
              <Link
                sx={{ color: 'blue' }}
                key={i}
                href={`https://twitter.com/hashtag/${segment
                  .replace('#', '')
                  .trim()}`}
                target="_blank"
                color="neutral"
